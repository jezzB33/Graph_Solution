...
# UPDATED: Added 'draw_maxel_graph' to the import list
...
st.markdown(r"Demonstrating $O(\log N)$ prediction and query optimization using generalized algebraic substrates.")
...
with tab1:
    ...
    st.markdown(r"Optimization relies on **Restriction Commutativity**: $\mathbf{M}_J^k = \mathbf{M}^k \cdot \mathbf{e}_J$.")
    ...
    with colC:
        ...
        # Draw the graph using the REAL function from maxel_core
        # Highlighting nodes in J
...
with tab2:
    ...
    st.markdown(r"Solves the linear system $\mathbf{M}\vec{x} = \vec{b}$ over the $\mathbb{R}$ field (real numbers) using algebraic inversion.")
    ...
    st.markdown(r"Maxel rank reduction guarantees $\mathbf{O(N^3)}$ time, avoiding the unpredictable nature of iterative search.")
...
# =======================================================================================
with tab3:
    st.header(r"🌊 Dynamic Vexel Calculus: $O(\log k)$ 'Warp Speed' Prediction")
    st.markdown(r"The $k$-th state $L_k$ is found by calculating the Maxel power $\mathbf{M}_q^{k-1}$ using $\mathbf{O(\log k)}$ steps.")
    ...
    plot_data_predicted = pd.DataFrame({
        ...
        'Type': r'Maxel Prediction ($O(\log k)$)'
    })
    ...
    st.caption(r"The Maxel framework calculates the distant state (large marker) algebraically in $O(\log k)$ time, bypassing all intermediate steps.")
    ...
    st.markdown(r"The **Maxel $O(\log k)$ time** line (yellow) visually flattens, confirming computation time is not linearly dependent on the forecast horizon.")
